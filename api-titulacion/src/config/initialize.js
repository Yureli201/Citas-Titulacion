...
const initializeApp = async () => {
  try {
    ...
    const tables = await db.query(`SELECT TABLE_NAME FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_TYPE = 'BASE TABLE' AND TABLE_SCHEMA = '${db.config.database}'`, { type: db.QueryTypes.SELECT });

    if (tables && tables.length === 0) {
      // La base de datos está vacía, crea las tablas
      await Cita.sync();
      await Egresado.sync();
      await fechasBloqueadas.sync();
