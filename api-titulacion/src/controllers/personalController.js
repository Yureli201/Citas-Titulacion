...
const mostrarFormularioEditarCita = async (req, res) => {
  try {
    const mat = req.params.mat; // Obtener el ID de la cita de los parámetros de la URL
    const cita = await Cita.findOne({where: {egresadoMatricula: mat}}); // Buscar la cita por su ID en la base de datos
...
const personalEditarCita = async (req, res) => {
  try {
    ...
    const { fecha, hora, estado } = req.body; // Obtener los nuevos datos de la cita del cuerpo de la solicitud
    ...
    // Redirigir a la página de administración de citas después de la edición
      res.json("Cita modificada con exito")
  } catch (error) {
...
const administrarPersonalController = async (req, res) => {
  try {
...
const editarEgresado = async (req, res) => {
  try {
    ...
    const EgresadoM = await Egresado.update(
    ...
    res.json("Egresado actulizado con exito")
...
const eliminarEgresado = async (req, res) => {
  try {
    ...
    res.json("Egresado eliminado con exito")
...
const editarPersonal = async (req, res) => {
  try {
    const numeroTrabajador = req.params.numeroTrabajador
    const { nombre } = req.body; // Obtener los nuevos datos del personal del cuerpo de la solicitud

    // Verificar si se ha proporcionado un nuevo número de trabajador
    if (numeroTrabajador) {
      // Actualizar el nombre del personal en la base de datos
      const Personal = await PersonalTitulacion.update(
        { nombre },
        {
          where: {
    ...
    res.json("Personal actualizado con exito")
...
const eliminarPersonal = async (req, res) => {
  try {
    ...
    res.json("Personal eliminado con exito")
...
const agregarNuevoEgresado = async (req, res) => {
  try {
    ...
    const { matricula, nombre, nivelTitulacion, generacion, carreraTitulacion, curp, correo, telefono } = req.body;
...
const registrarPersonal = async (req, res) => {
  try {
    ...
    const personalC = await PersonalTitulacion.create({
      numeroTrabajador,
      nombre,
    });
...
export { aministrarCitasController, mostrarFormularioEditarCita, personalEditarCita, personalEliminarCita, administrarUsuariosController, administrarPersonalController, mostrarFormularioEditarEgresado, editarEgresado, eliminarEgresado, mostrarFormularioEditarPersonal, editarPersonal, eliminarPersonal, agregarNuevoEgresado, renderAgregarEgresado, renderAgregarPersonal, registrarPersonal };
